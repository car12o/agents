---
name: plan-review
description: Review a plan doc generated by the plan-doc skill using multiple AI agents. Use when asked to review, audit, or critique a plan written by plan-doc.
...
ls "$root"/.agents/plans/*.md 2>/dev/null | tail -1
```

If the command prints nothing, no plan exists: stop and tell the user. A plan whose filename carries a `-NN-` sequence is one of a split set; tell the user which siblings exist so they can review each one.
...
Check for: missing steps, wrong sequencing, unrealistic size estimates, steps larger than M (one day), steps without a verification condition, steps that cannot be independently reviewed, tests from Testing Strategy or code-level Rollout & Migration items that no step implements.
