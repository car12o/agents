...
| **git-conventions** | ✅ | Shared git ruleset: default-branch detection (with fallback, never guess) and no direct commits or pushes to it, `<type>/<slug>` branch naming and worktree-safe creation from an up-to-date default (name checked locally and on origin), Conventional Commits with the prefix table, commit hygiene (one logical change per commit, only files you touched, staged by path, hooks never bypassed), push rules (stop on rejection, never force), PR title/description rules, and the no-attribution rule. Loaded automatically before creating a branch, committing, pushing, or opening a PR; the other git-touching skills reference it instead of restating it. |
