...
The `plan-doc → plan-review → plan-implement` skills form a pipeline: draft a plan, get it adversarially reviewed and refined, then execute it. `git-flow` covers the branch → commit → PR steps on their own and closes the pipeline: `plan-implement` leaves its commits local and hands off to `git-flow pr`. Both, and `multi-code-review`, defer to `git-conventions` for branch, commit, and PR rules.
