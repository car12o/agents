...
| **review-fanout** | ✅ | Shared fan-out protocol for the two review skills: private prompt directory and cleanup, agent selection, one background `ask-agent` call per agent, exit-code and failure classes (timeout / error / empty / narration-only), the orchestrator's own review while agents run, deduplication into one canonical finding with its sources, and the per-agent coverage line. Loaded by `plan-review` and `multi-code-review`; they reference it instead of restating it. |
| **git-conventions** | ✅ | Shared git ruleset: default-branch detection (with fallback, never guess), `<type>/<slug>` branch naming and creation from an up-to-date default, Conventional Commits with the prefix table, commit hygiene (one logical change per commit, only files you touched, staged by path), PR title/description rules, and the no-attribution rule. Loaded automatically before creating a branch, committing, or opening a PR; the other git-touching skills reference it instead of restating it. |
| **git-flow** | ❌ (explicit) | Stepped git flow: create a `<type>/<slug>` feature branch from an up-to-date default branch, commit already-staged changes (one commit per logical change) and push, then open a PR against the default branch (pushing first if the branch has no upstream). Run all three steps or select a subset by number or name (`branch`, `commit`, `pr`); selected steps always run in order. |

The `plan-doc → plan-review → plan-implement` skills form a pipeline: draft a plan, get it adversarially reviewed and refined, then execute it. `git-flow` covers the branch → commit → PR steps on their own and closes the pipeline: `plan-implement` leaves its commits local and hands off to `git-flow pr`. Both, and `multi-code-review`, defer to `git-conventions` for branch, commit, and PR rules; `plan-review` and `multi-code-review` defer to `review-fanout` for the fan-out protocol.
