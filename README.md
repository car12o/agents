...
| **plan-implement** | ❌ (explicit) | Implements the most recent (or specified) plan doc. Locates the plan, confirms prerequisite plans are merged and no blocking question is open, creates a `<type>/<slug>` feature branch from the plan header if on the default branch, and commits one logical change per step following `git-conventions`. |
